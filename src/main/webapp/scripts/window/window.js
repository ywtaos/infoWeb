	...
	var weebox = function(content,options) {
		...
		this.initBox = function() {
			...
			switch(self.options.type) {
				...
				html =  '<div class="weedialog">' +
						'	<div class="dialog-header"><div id="window_title"></div>' +
						'		<div class="dialog-tc2"><a href="javascript:;" onclick="return false" title="关闭" class="dialog-close"></a></div>' +
						'	</div>' +
						...
						'					<a class="dialog-ok">确定</a>' +
						'					<a class="dialog-cancel">取消</a>' +
						'				</div>' +
						'			</td>' +
			...
			if (self.options.width>0) {
				...
				$("#window_title").css('width',self.options.width-48);
			}
			self.dh.bgiframe();
		};
		...
		this.setOncancel = function(fn) {
			...
			if (typeof(fn)=="function"){
				self.dh.find(".dialog-cancel").click(function(){fn(self);});	
			}
		};
		//设置onOnclose事件
		...
		this.bheight = function() {
			if ($.support.leadingWhitespace==false && $.support.noCloneEvent==false) {
		...
		this.bwidth = function() {
			if ($.support.leadingWhitespace==false && $.support.noCloneEvent==false) {
				...
				if (scrollWidth < offsetWidth) {
					return $(window).width() - 22;
				} else {
					return scrollWidth - 22;
				}
			} else {
				return $(document).width() - 22;
			}
		};
		//将弹窗显示在中间位置
